...
set temp_ini 0.025; #initial temperature
set temp_fin 0.050; #final temperature
set dtemp 0.025 ;#temperature step
set Ntemp [expr (($temp_fin-$temp_ini)/$dtemp)+1]

#barostat and thermostat data
set press 0.01 ;#pressure for the barostat
...
set box_lx 10.0 ;# size in the x-direction
...
#capped interaction
set integ_steps_cap 500
set cap 5.0
set cap_max 500.0
#warm up
set integ_steps_warm 10000
#production cycles
...
#CS interacion
inter $water $water tabulated potential.dat
########################## insert the particles
for { set i 0 } { $i < $Npart } {incr i 1} {
    set posx [expr $box_lx*[t_random]]
    set posy [expr $box_ly*[t_random]]
    set posz [expr $box_lz*[t_random]]
    part $i pos $posx $posy $posz q 0 type $water 
}
