...
set temp_ini 0.100; #initial temperature
set temp_fin 0.100; #final temperature
set dtemp 0.005 ;#temperature step
set Ntemp [expr (($temp_fin-$temp_ini)/$dtemp)+1]

#barostat and thermostat data
set press 1.25 ;#pressure for the barostat
...
set box_lx 100.0 ;# size in the x-direction
...
for { set i 0 } { $i < [expr 2.*$Npart] } {incr i 2} {
...
for { set Tcount 0 } { $Tcount < $Ntemp} {incr Tcount 1} {
    ...
    set temp  [format %1.4g [expr $temp_ini+$Tcount*$dtemp]]
